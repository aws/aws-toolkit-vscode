            ...
            try {
                ...
                if (
                    ...
                    const response = await vscode.window.showInformationMessage(
                        `Unable to use your console credentials for profile "${this.profileName}". Would you like to retry?`,
                        localizedText.retry,
                    ...
                    try {
                        ...
                        void vscode.window.showErrorMessage(
                            `Unable to refresh your AWS credentials. Please run 'aws login --profile ${this.profileName}' in your terminal, then reload VS Code to continue.`
                        )
                    }
                }
                ...
                if (requiresVscodeReloadForCredentials) {
                    getLogger().info(
                        `Reloading window to sync with updated credentials cache using connection for profile: ${this.profileName}`
                    )
                    const reloadResponse = await vscode.window.showInformationMessage(
                        `Credentials for "${this.profileName}" have been updated. Reload window to apply?`,
                        localizedText.yes,
                        localizedText.no
                    )
