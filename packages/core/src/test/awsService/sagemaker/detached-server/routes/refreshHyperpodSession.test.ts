...
import { handleRefreshHyperpodSession } from '../../../../../awsService/sagemaker/detached-server/routes/refreshHyperpodSession'
...
describe('handleRefreshHyperpodSession', function () {
    ...
    function createMockRequest(queryString: string): IncomingMessage {
        return { url: `/refresh_hyperpod_session${queryString}` } as IncomingMessage
    }
    ...
    it('returns 400 when connection_key is missing', async function () {
        ...
        await handleRefreshHyperpodSession(req, res as unknown as ServerResponse)
    ...
    it('returns 200 with session info on successful workspace connection', async function () {
        ...
        sinon
            .stub(kubectlClientStubModule.KubectlClient.prototype, 'createWorkspaceConnection')
    ...
    it('returns 500 when createWorkspaceConnection fails', async function () {
        ...
        sinon
            .stub(kubectlClientStubModule.KubectlClient.prototype, 'createWorkspaceConnection')
            .rejects(new Error('K8s API timeout'))
