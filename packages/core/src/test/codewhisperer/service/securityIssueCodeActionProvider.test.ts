...
describe('securityIssueCodeActionProvider', () => {
    ...
    it('should provide quick fix for each issue that has a suggested fix', () => {
        ...
        assert.strictEqual(actual[0].title, 'Fix with Amazon Q')
        ...
        assert.strictEqual(actual[4].title, 'View details for "issue 2"')
        assert.strictEqual(actual[4].kind, undefined)
        assert.strictEqual(actual[5].title, 'Explain with Amazon Q')
