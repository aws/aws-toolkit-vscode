...
class FakeClient implements AwsClient {
    constructor(public readonly config: AwsClientOptions) {}
...
describe('DefaultAwsClientBuilderV3', function () {
    ...
    describe('createAndConfigureSdkClient', function () {
        it('includes Toolkit user-agent if no options are specified', async function () {
            const service = await builder.createAwsService(FakeClient)
        ...
        describe('request listeners', function () {
            ...
            it('calls listener with correct type', async function () {
                const service = await builder.createAwsService(FakeClient, {
                    ...
                    onRequestSetup: [
                        (request: any) => {
                            assert.ok(request.service instanceof FakeClient)
            ...
            it('can add listeners without affecting the original class', async function () {
                ...
                const base = class extends FakeClient {
                ...
                function factory(...args: any[]) {
                    const instance = new FakeClient(...args)
                ...
                assert.ok(service instanceof FakeClient)
