...
for (let i = 0; i < 10; i++) {
    describe('Amazon Q Test Generation', function () {
        ...
        async function setupTestDocument(filePath: string, language: string) {
            console.log('1a: started opening test file')
            ...
            console.log('1b: opened file and now need to show file')
            ...
            console.log('1c: finished opening test file')
        }
        ...
        async function cleanupTestFile(testFilePath: string) {
            console.log('4a: started cleaning test file')
            ...
            console.log('4b: finished cleaning test file')
        }
        ...
        afterEach(async () => {
            ...
            framework.dispose()
            sinon.restore()
            console.log('Test passed: finished afterEach for this test')
        })
            ...
            for (const { language, filePath, testFilePath } of testFiles) {
                describe(`/test on ${language} file`, () => {
                    beforeEach(async () => {
                        ...
                        console.log(`2a: called /test on ${language} file`)
                        tab.addChatMessage({ command: '/test' })
                        await tab.waitForChatFinishesLoading()

                        console.log('2b: waiting for view diff button')
                        await tab.waitForButtons([FollowUpTypes.ViewDiff])

                        console.log('2c: clicked on view diff button')
                        tab.clickButton(FollowUpTypes.ViewDiff)
                        await tab.waitForChatFinishesLoading()
                        console.log('2d: chat finished loading after clicking on view diff')
                    })

                    describe('View diff of test file', async () => {
                        it('Clicks on view diff', async () => {
                            console.log('3a: started view diff test')
                            ...
                            console.log('3b: finished view diff test')
                        })
                    })
                    ...
                    describe('Accept unit tests', async () => {
                        ...
                        it('Clicks on accept', async () => {
                            console.log('3a: started clicks on accept test')
                            ...
                            console.log('3b: finished clicks on accept test')
                        })
                    })

                    describe('Reject unit tests', async () => {
                        it('Clicks on reject', async () => {
                            console.log('3a: started clicks on reject test')
                            await tab.waitForButtons([FollowUpTypes.AcceptCode, FollowUpTypes.RejectCode])
                            ...
                            console.log('3b: finished clicks on reject test')
                        })
                    })
                })
            }
