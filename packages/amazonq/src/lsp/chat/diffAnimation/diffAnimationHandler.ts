...
export class DiffAnimationHandler implements vscode.Disposable {
    ...
    /**
     * Common logic for initializing streaming sessions
     */
    private async initializeStreamingSession(
        ...
        if (!this.isStreamingActive(streamingChunk.toolUseId) && filePath && !isAlreadyInitializing) {
            ...
            try {
                ...
                getLogger().error(`${errorPrefix} ${streamingChunk.toolUseId}: ${error}`)
                if (errorPrefix.includes('fsReplace')) {
                    // Don't rethrow for fsReplace
                } else {
                    throw error
                }
            } finally {
