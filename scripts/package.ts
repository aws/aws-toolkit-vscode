...
/**
 * Checks for npm errors in a given output string.
 * Ignores lines that are not specific errors as well as any errors related to src.gen/
 * because that code is automatically generated.
 */
function hasBlockingNpmErrors(logOutput: string): boolean {
    ...
    for (const l of lines) {
        if (l.startsWith('npm ERR! code ELSPROBLEMS')) {
            // Beginning of the error list.
            // If the code is something else, then it is significant.
            continue
        } else if (l.startsWith('npm ERR! A complete log')) {
            // Done reporting errors at this point
            break
        } else if (!l.includes('src.gen/')) {
            // Ignore src.gen/
            // Other errors are significant
            return true
        }
    }
...
function main() {
    ...
    try {
        ...
        const npmListCmd = 'npm'
        const npmListArgs = ['list', '--production', '--parseable', '--depth=99999', '--loglevel=error']
        try {
            child_process.execFileSync(npmListCmd, npmListArgs, {
                stdio: 'pipe',
            })
        } catch (err: any) {
            const output = err.stderr.toString()
            if (hasBlockingNpmErrors(output)) {
                console.log(
                    `Error: Blocking NPM errors found while running '${npmListCmd} ${npmListArgs.join(' ')}'. Please resolve errors NOT related to src.gen/. See full output:\n${output}`
                )
                throw new Error('vsce package prerequisite `npm list` detected errors not related to srg.gen/')
            }
        }
        ...
        child_process.execFileSync(
            ...
            [
                ...
                '--no-dependencies', // Prevents running `npm list ...`, which we validate ourselves above. Side effects? https://github.com/microsoft/vscode-vsce/issues/439
            ],
            {
                stdio: 'inherit',
