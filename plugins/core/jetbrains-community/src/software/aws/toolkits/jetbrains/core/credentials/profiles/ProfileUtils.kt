...
fun Profile.traverseCredentialChain(profiles: Map<String, Profile>): Sequence<Profile> = sequence {
    ...
    while (currentProfile.propertyExists(ProfileProperty.ROLE_ARN)) {
        ...
        if (!profileChain.add(currentProfileName)) {
            ...
            throw IllegalArgumentException(AwsCoreBundle.message("credentials.profile.circular_profiles", name(), chain))
        }
        ...
        if (sourceProfile.isPresent && credentialSource.isPresent) {
            throw IllegalArgumentException(AwsCoreBundle.message("credentials.profile.assume_role.duplicate_source", currentProfileName))
        }
        ...
        if (sourceProfile.isPresent) {
            ...
            throw IllegalArgumentException(AwsCoreBundle.message("credentials.profile.assume_role.missing_source", currentProfileName))
        }
    }
}
