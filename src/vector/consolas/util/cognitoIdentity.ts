...
export const getCognitoCredentials = async (): Promise<CognitoIdentityCredentials> => {
    const region = ConsolasConstants.region
    try {
        // grab Cognito identityId
        const poolId = ConsolasConstants.prodIdentityPoolID
        ...
        getLogger().error(`Failed to initialize Cognito identity for Consolas: ${err} in region: ${region}`)
        return Promise.reject(`Failed to initialize Cognito identity for Consolas: ${err}`)
    }
}
...
const getIdentityFromIdentityPool = async (): Promise<string> => {
    try {
        const res = await new CognitoIdentity({
            ...
            .getId({
                IdentityPoolId: ConsolasConstants.prodIdentityPoolID,
            })
            .promise()
        ...
        return Promise.reject(`Failed to get new Cognito identity for Consolas: ${err}`)
    }
}
