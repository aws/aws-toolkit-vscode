...
export abstract class DependencyGraph {
    ...
    protected reachSizeLimit(size: number) {
        return size > ConsolasConstants.codeScanPayloadSizeLimit
    }
    ...
    protected getTruncDirPath(uri: vscode.Uri) {
        if (this._truncDir === '') {
            this._truncDir = path.join(
                this._tmpDir,
                ConsolasConstants.codeScanTruncDirPrefix + '_' + new Date().getTime().toString()
            )
        }
