...
function makeUri(...args: Parameters<typeof openEditorAtRange>): vscode.Uri {
    return vscode.Uri.parse(`command:aws.consolas.openEditorAtRange?${encodeURIComponent(JSON.stringify(args))}`)
}
...
vscode.commands.registerCommand('aws.consolas.openEditorAtRange', openEditorAtRange)

export class SecurityPanelViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'aws.consolas.securityPanel'
