...
import { ConfigurationEntry, vsCodeState } from '../../../../vector/consolas/models/model'
...
describe('inlineCompletion', function () {
    ...
    describe('acceptRecommendation', async function () {
        ...
        it('should not call editor.edit when consolas is editing', async function () {
            const mockEditor = createMockTextEditor()
            invokeSpy = sinon.stub(mockEditor, 'edit')
            // Set isConsolasEditing to true
        ...
        it('should reset ConsolasConstants.serviceActiveKey after acceptance', async function () {
            const mockEditor = createMockTextEditor()
            ...
            await InlineCompletion.instance.acceptRecommendation(mockEditor)
            const ServiceKey = globals.context.globalState.get<boolean>(ConsolasConstants.serviceActiveKey) || false
            assert.ok(invokeSpy.called)
    ...
    describe('rejectRecommendation', async function () {
        ...
        it('should reset ConsolasConstants.serviceActiveKey after rejectRecommendation', async function () {
            const mockEditor = createMockTextEditor()
            invokeSpy = sinon.stub(mockEditor, 'edit').resolves()
            const mockPosition = new vscode.Position(0, 0)
            InlineCompletion.instance.setRange(new vscode.Range(mockPosition, mockPosition))
    ...
    describe('navigateRecommendation', async function () {
        ...
        it('should not call showRecommendation when consolas is editing', async function () {
            const mockEditor = createMockTextEditor()
            // sinon.restore()
