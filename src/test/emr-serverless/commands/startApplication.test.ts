...
import { startApplication } from '../../../emr-serverless/commands/startApplication'
import { FakeCommands } from '../../shared/vscode/fakeCommands'
import { getTestWindow } from '../../shared/vscode/window'

describe('startApplicationCommand', function () {
    const appId = 'app01'
    ...
    it('starts the application, shows progress bar, and refreshes parent node', async function () {
        const commands = new FakeCommands()
        const stub = sandbox.stub(emrServerlessClient, 'startApplication').callsFake(async id => {
            assert.strictEqual(id, appId)
        })

        await startApplication(node, commands)

        getTestWindow().getFirstMessage().assertProgress(`Starting ${appId}...`)
