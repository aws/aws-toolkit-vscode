...
export class MockPrompter<T> extends Prompter<T> {
    ...
    protected async promptUser(): Promise<PromptResult<T>> { return this.input }
    public setLastResponse(picked?: any): void {}
...
describe('Prompter', function () {
    it('returns a value', async function() {
        const prompter = new MockPrompter(1)
        assert.strictEqual(await prompter.prompt(), 1)
    })

    it('can map one type to another', async function() {
        const prompter = new MockPrompter(1).transform(resp => resp.toString())
        assert.strictEqual(await prompter.prompt(), '1')
    })
    ...
    it('multiple callbacks can be attached', async function() {
        const prompter = new MockPrompter(1)
