...
describe('WizardForm', function () {
    ...
    it('can add prompter', function () {
        testForm.body.prop1.bindPrompter(() => new MockPrompter(0))
    ...
    it('form values will use default setting if not assigned', function () {
        testForm.body.prop1.bindPrompter(() => new MockPrompter(0), { setDefault: () => 100 })
        tester.prop1.assertShow()
        tester.prop1.assertValue(100)
        tester.prop1.applyInput(5)
        tester.prop1.assertDoesNotShow()
        tester.prop1.assertValue(5)
        tester.prop1.removeInput()
        tester.prop1.assertShow()
    })

    // TODO: revisit this. values should not technically know about other field defaults
    // but allowing this behavior means loops are possible (use proxy?)
    ...
    describe('requireParent', function () {
        it('prompters only show when parent is defined if set', function () {
            testForm.body.nestedProp.prop1.bindPrompter(() => new MockPrompter(''), { requireParent: true })
            testForm.body.nestedProp.prop2.bindPrompter(() => new MockPrompter(''))
        ...
        it('works with "showWhen"', function () {
            testForm.body.nestedProp.prop1.bindPrompter(() => new MockPrompter(''), { 
                requireParent: true,
    ...
    describe('addForm', function () {
        ...
        it('can apply another form to a property', function () {
            nestedTestForm.body.prop1.bindPrompter(() => new MockPrompter(''))
            testForm.body.nestedProp.applyForm(nestedTestForm)
            tester.nestedProp.prop1.assertShow()
        })

        it('nested forms can use local state', function () {
            nestedTestForm.body.prop1.bindPrompter(() => new MockPrompter(''), 
                { showWhen: state => state.prop2 === 'hello' }
            )
            testForm.body.nestedProp.applyForm(nestedTestForm)
        ...
        it('can apply form with "showWhen"', function () {
            ...
            nestedTestForm.body.prop2.bindPrompter(() => new MockPrompter(''), 
                { showWhen: state => state.prop1 === 'goodbye' }
            )

            testForm.body.nestedProp.applyForm(nestedTestForm, { showWhen: state => state.prop2 === 'start' })
            tester.nestedProp.prop1.assertDoesNotShow()
            tester.nestedProp.prop2.assertDoesNotShow()
            tester.nestedProp.applyInput({})
            tester.nestedProp.prop1.assertDoesNotShow()
            tester.nestedProp.prop2.assertDoesNotShow()
            tester.nestedProp.removeInput()
            tester.prop2.applyInput('start')
            tester.nestedProp.prop1.assertDoesNotShow()
            tester.nestedProp.prop2.assertDoesNotShow()
            tester.nestedProp.prop1.applyInput('goodbye')
            tester.nestedProp.prop2.assertShow()
            tester.nestedProp.prop1.removeInput()
            tester.nestedProp.prop2.applyInput('hello')
            tester.nestedProp.prop1.assertShow()
        })

        it('can apply form with "setDefault"', function () {
            nestedTestForm.body.prop1.bindPrompter(() => new MockPrompter(''), { requireParent: true })
            nestedTestForm.body.prop2.setDefault(state => `${state.prop1}.${state.prop1}`)
