...
export class SamVisualizationManager {
    ...
    public readonly managedVisualizations: Map<string, SamVisualization> = new Map<string, SamVisualization>()
     ...
     * @returns
     */
    public renderSamVisualization(activeTextEditor: vscode.TextEditor | undefined): vscode.WebviewPanel | undefined {
        ...
        if (!activeTextEditor || activeTextEditor.document.fileName.includes('extension-output')) {
            logger.error('Could not get active text editor for SAM Template render.')
            throw new Error('Could not get active text editor for SAM Template render.')
        }
