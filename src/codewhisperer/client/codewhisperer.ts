...
import { AWSError, CognitoIdentityCredentials, Credentials, Service } from 'aws-sdk'
...
export class DefaultCodeWhispererClient {
    ...
    private async createSdkClient(): Promise<CodeWhispererClient> {
        ...
        return (await globals.sdkClientBuilder.createAwsService(Service, {
            apiConfig: apiConfig,
            ...
            onRequestSetup: [
                req => {
                    ...
                    if (isCloud9() && req.operation !== 'getAccessToken') {
                        req.on('retry', resp => {
                            if (resp.error?.code === 'AccessDeniedException' && resp.error.message.match(/expired/i)) {
                                if (req.service.config.credentials instanceof Credentials) {
                                    resp.error.retryable = true
                                    req.service.config.credentials.expired = true
                                }
                            }
                        })
                    }
