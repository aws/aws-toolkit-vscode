...
export class cloudformationDependencyGraph extends DependencyGraph {
    // Payload Size for JSON/YAML: 200KB
    getPayloadSizeLimitInBytes(): number {
        return CodeWhispererConstants.codeScanCFPayloadSizeLimitBytes
    }
    override async traverseDir(dirPath: string) {
        ...
        readdirSync(dirPath, { withFileTypes: true }).forEach(async file => {
            ...
            if (file.isDirectory()) {
                ...
                if (
                    // Check for file extension: .json or .yaml or .yml
                    (file.name.endsWith('.' + this._languageId) ||
                        (file.name.endsWith(DependencyGraphConstants.ymlExt) &&
                            this._languageId === ('yaml' satisfies CodeWhispererConstants.PlatformLanguageId))) &&
