...
export class InlineCompletionService {
    private maxPage = 100
    private statusBar: CodeWhispererStatusBar = CodeWhispererStatusBar.instance
    ...
    private async setState(state: keyof typeof states) {
        ...
        switch (state) {
            ...
            case 'ok': {
                this.statusBar.setState('ok', CodeSuggestionsState.instance.isSuggestionsEnabled())
                break
            }
            case 'expired': {
                this.statusBar.setState('expired')
                break
            }
            case 'notConnected': {
                this.statusBar.setState('notConnected')
                break
            }
        }
    }
