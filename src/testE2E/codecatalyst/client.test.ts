...
describe('Test how this codebase uses the CodeCatalyst API', function () {
    ...
    describe('Dev Environment functionality', function () {
        ...
        after(async function () {
            ...
            //         break
            //     } catch (e: any) {
            //         console.log(`call failed, code: ${e.code}`)
            //         if (e.code === 'ThrottlingException' && attempt < 5) {
            //             console.log(`throttled attempt: ${attempt}`)
            //             attempt += 1
            //             const waitTime = Math.random() * (Math.pow(2, attempt) * 1200)
    ...
    async function createTestCodeCatalystClient(auth: Auth): Promise<CodeCatalystClient> {
        ...
        return await createCodeCatalystClient(conn, undefined, undefined, {
            ...
            retryDelayOptions: {
                ...
                //     const time = retryCount <= 5 ? Math.random() * (Math.pow(2, retryCount) * 2000) : -1
                //     getLogger().debug(`retry request: ${retryCount} @ ${time} ms`)
                //     return time
                // },
            },
            maxRetries: 5,
    ...
    async function tryCreateTestProject(
        ...
        try {
            getLogger().debug(`trying to make: ${spaceName}/${projectName}`)
            ...
            getLogger().error(`failed to create: ${spaceName}/${projectName}`)
            if ((isAwsError(e) || e instanceof ToolkitError) && e.code === 'ConflictException') {
                getLogger().error(`Tried to create test project but it already exists: ${spaceName}/${projectName}`)
