...
export interface SdkDefService {
    ...
    operations: Record<string, SdkDefServiceCall>
    shapes: Record<string, SdkDefServiceCallShape>
}
...
export class SdkDefs {
    ...
    private static INSTANCE: SdkDefs | undefined

    // TODO: make this not a singleton once the webview can accept external contexts?
    public static getInstance(ctx?: ExtContext): SdkDefs {
        if (!SdkDefs.INSTANCE) {
            if (!ctx) {
                throw new Error('Uninitialized SdkDefs requires an ExtContext object!')
            }
            SdkDefs.INSTANCE = new SdkDefs(ctx)
        }

        return SdkDefs.INSTANCE
    }

    private constructor(private readonly ctx: ExtContext) {
        this.VERSION = `v${'2.916.0'}` // TODO: Find out how to get the SDK version from package.json
    ...
    public async getAvailableServices(): Promise<string[]> {
        if (!(await this.isSdkDefsReady())) {
            throw new Error('getAvailableServices failed: SDK defs not ready')
        }

        return Object.keys(this.sdkDefs)
    }
     ...
     */
    public async getServiceData(service: string): Promise<SdkDefService | undefined> {
        if (!(await this.isSdkDefsReady())) {
            throw new Error('getServiceData failed: SDK defs not ready')
        }
        try {
            ...
            return this.sdkDefs[service] as SdkDefService
        } catch (e) {
