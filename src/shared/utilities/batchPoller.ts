...
interface BatchPollerSettings {
    ...
    logging: boolean
    /**
     * Determines how the poller should choose the next poll time from a sorted array of potential
...
function median(deltas: number[]): number {
    const half = Math.floor(deltas.length / 2)
    return deltas.length % 2 ? deltas[half] : (deltas[half - 1] + deltas[half]) / 2.0
}
...
export class BatchPoller<Model> {
    ...
    protected updateCollisions(): number {
        const deltas = [...this.listenerPool.values()].map(element => this.updateElement(element) - Date.now()).sort()
