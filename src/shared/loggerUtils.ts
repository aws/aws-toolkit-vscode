...
export class TestLogger {
    ...
    public async logContainsText(str: string): Promise<boolean> {
        const logText = await filesystemUtilities.readFileAsString(this.logfilePath)
    ...
    public static async createTestLogger(): Promise<TestLogger> {
        ...
        const logFilePath = path.join(logFolder, 'temp.log')
        const logger = await l.initialize({
            logPath: logFilePath,
            logLevel: 'debug'
        })

        return new TestLogger(logFolder, logFilePath, logger)
    }
}
