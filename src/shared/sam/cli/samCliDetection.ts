...
const browseToSamCli = localize(
    'AWS.samcli.userChoice.browse',
    'Locate SAM CLI...'
)

const settingsUpdated = localize(
    'AWS.samcli.detect.settings.updated',
    'Settings updated.'
)

const settingsNotUpdated = localize(
    'AWS.samcli.detect.settings.not.updated',
    'No settings changes necessary.'
)
    ...
    await lock.acquire('detect SAM CLI', async () => {
        ...
        if (showMessageIfDetected) {
            if (!currentsamCliLocation) {
                ...
                const message: string = (initialSamCliLocation === currentsamCliLocation) ?
...
function getSettingsUpdatedMessage(location: string): string {
    const configuredLocation = localize(
        'AWS.samcli.configured.location',
        'Configured SAM CLI Location: {0}',
        location
    )
