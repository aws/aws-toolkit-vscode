...
export const WIZARD_RETRY: WizardControl = { id: WIZARD_CONTROL, type: StateMachineControl.Retry }
export const WIZARD_BACK: WizardControl = { id: WIZARD_CONTROL, type: StateMachineControl.Back }
export const WIZARD_EXIT: WizardControl = { id: WIZARD_CONTROL, type: StateMachineControl.Exit }
...
export abstract class Wizard<TState extends WizardSchema<TState>, TResult=TState> {
    private readonly formData = new Map<string, StepWithOptions<TState, any>>()
    ...
    public async run(): Promise<TState | TResult | undefined> {
        this.resolveNextSteps(this.schema as TState).forEach(step => this.stateController.addStep(step))
    ...
    private createBindPrompterMethod<TProp>(prop: string): PrompterBind<TProp, TState> {
        return (
            ...
            const boundStep = async (state: TState): Promise<StateMachineStepResult<TState>> => {
                // TODO: apply a default prototype to the state
                const stateWithCache = state as StateWithCache<TState>
                stateWithCache.stepCache = stepCache
                const response = await this.promptUser(stateWithCache, prompterProvider(stateWithCache))
                _.set(state, prop, response)
                delete (stateWithCache as Partial<StateWithCache<TState>>)['stepCache']
    ...
    private createWizardForm(schema: any, path: string[] = []): WizardForm<TState> {
        ...
        Object.entries(schema).forEach(([key, value]: [string, unknown]) => {
            ...
            const element = {
                bindPrompter: this.createBindPrompterMethod(newPath.join('.')),
