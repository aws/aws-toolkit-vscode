...
export class DefaultAWSClientBuilder implements AWSClientBuilder {
    ...
    public async createAndConfigureServiceClient<T>(
        ...
        if (!awsServiceOpts.customUserAgent) {
            ...
            const platformName = ext.vscode.env.appName.replace(/\s/g, '-')
            awsServiceOpts.customUserAgent =
                `AWS-Toolkit-For-VisualStudio/${pluginVersion} ${platformName}/${ext.vscode.version}`
        }
