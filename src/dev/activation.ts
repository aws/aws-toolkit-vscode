...
async function installVsix(
    ...
    new SessionProcess('ssh', [baseUri, '-v']).run()
    await new Promise<void>(r => setTimeout(r, 5000))

    progress.report({ message: 'Copying VSIX...' })
    await new SessionProcess('scp', ['-v', resp, `mde-user@${baseUri}:${remoteVsix}`]).run()

    // extensions/.obsolete is JSON map, need to remove extension ID if present
    // you have to move `extension.vsixmanifest` somewhere? I guess into the new dir as `.vsixmanifest`
    // need to clean up old installed extensions otherwise it won't pick up the new one
    //  Exit status 0 <-- used to see if cmd failed?
    ...
    const envScript = `
    # Generated by the AWS Toolkit for VS Code
    # https://github.com/aws/aws-toolkit-vscode
    export AWS_TOOLKIT_ENVIRONMENT_ID="${env.id}"
    `
    const echoScript = `echo '${envScript}' >> ~/.profile`

    progress.report({ message: 'Adding ".profile" script...' })
    await new SessionProcess('ssh', [baseUri, '-v', echoScript]).run()
