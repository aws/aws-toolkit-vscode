...
class CodeWhispererYamlCodeScanTest : CodeWhispererCodeScanTestBase(PythonCodeInsightTestFixtureRule()) {
    ...
    override fun setup() {
        super.setup()
        setupYamlProject()
        sessionConfigSpy = spy(CodeScanSessionConfig.create(testYaml, project) as CloudFormationYamlCodeScanSessionConfig)
    ...
    fun `test getSourceFilesUnderProjectRoot`() {
        assertThat(sessionConfigSpy.getSourceFilesUnderProjectRoot(testYaml).size).isEqualTo(3)
    }
