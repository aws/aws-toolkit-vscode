...
class CodeWhispererModelConfiguratorTest {
    ...
    fun `loadState should load the correct values into memory`() {
        ...
        val conn1 = spy(ManagedBearerSsoConnection(region = "us-east-1", startUrl = "url 1", scopes = CODEWHISPERER_SCOPES))
        val conn2 = spy(ManagedBearerSsoConnection(region = "us-east-2", startUrl = "url 2", scopes = CODEWHISPERER_SCOPES))
    ...
    fun `switchCustomization takes no effect if user is using builder id`() {
        credManager.clear()
        val builderIdConn = spy(ManagedBearerSsoConnection(region = SONO_REGION, startUrl = SONO_URL, scopes = CODEWHISPERER_SCOPES))
    ...
    fun `switchCustomization will update customization for identityCenter users`() {
        ...
        val ssoConn = spy(ManagedBearerSsoConnection(region = "us-east-1", startUrl = "url 1", scopes = CODEWHISPERER_SCOPES))
    ...
    fun `invalidateCustomization should remove all`() {
        val conn1 = spy(ManagedBearerSsoConnection(region = "us-east-1", startUrl = "url 1", scopes = CODEWHISPERER_SCOPES))
        val conn2 = spy(ManagedBearerSsoConnection(region = "us-east-1", startUrl = "url 1", scopes = CODEWHISPERER_SCOPES))
        val conn3 = spy(ManagedBearerSsoConnection(region = "us-east-2", startUrl = "url 2", scopes = CODEWHISPERER_SCOPES))
    ...
    fun `listCustomization return null if buildId connection`() {
        ...
        val builderIdConn = ManagedBearerSsoConnection(region = SONO_REGION, startUrl = SONO_URL, scopes = CODEWHISPERER_SCOPES)
