...
export const se_ChatCommand = async (input: ChatCommandInput, context: __SerdeContext): Promise<__HttpRequest> => {
    ...
    let resolvedPath = `${basePath?.endsWith('/') ? basePath.slice(0, -1) : basePath || ''}` + '/chat'
    let body: any
    body = JSON.stringify(
        take(input, {
...
export const se_StartConversationCommand = async (
    ...
    let resolvedPath = `${basePath?.endsWith('/') ? basePath.slice(0, -1) : basePath || ''}` + '/start-conversation'
    let body: any
...
export const de_ChatCommand = async (
    ...
    contents.chatResponse = de_ChatResponseStream(data, context)
    return contents
}

/**
 * deserializeAws_restJson1ChatCommandError
 */
